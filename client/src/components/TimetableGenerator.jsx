    ...
    for (let d = 0; d < 6; d++) {
      ...
      sectionsArr.forEach(sec => {
        for (let s = 0; s < currentDaySlots; s++) {
          if (globalTimetables[sec][d][s] !== null) continue;
          
          const dayHasSpecial = globalTimetables[sec][d].some(c => c?.toLowerCase().includes("lab") || c?.toLowerCase().includes("nss"));
          const getWeeklyUsage = (sub) => globalTimetables[sec].flat().filter(c => c === sub).length;

          let options = subList.filter(sub => {
            const name = sub.toLowerCase();
            if (name.includes("nss") && s < 4) return false; 
            if ((name.includes("lab") || name.includes("nss")) && (getWeeklyUsage(sub) >= 1 || dayHasSpecial)) return false;
            if ((name.includes("lab") || name.includes("nss")) && s + 1 >= currentDaySlots) return false;
            return !sectionsArr.some(other => globalTimetables[other][d][s] === sub);
          });

          const sel = options.length > 0 ? Random.shuffle(options)[0] : subList[0];
          globalTimetables[sec][d][s] = sel;
          if (sel.toLowerCase().includes("lab") || sel.toLowerCase().includes("nss")) {
            if (s + 1 < currentDaySlots) globalTimetables[sec][d][s+1] = "SKIP";
          }
        }
      });
    }
